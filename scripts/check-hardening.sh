...
# Round 2 (maintainer review of 331a883, 2026-08-27):
#   5. every FileView must set preload: false — content is read through
#      BoundedFile (regular-file gate + nonblocking + byte cap), never
#      through FileView's own load, which has no byte or file-type bound
#   6. no `cat >` redirection writes — flag writes are mktemp + atomic mv
#      in the target directory (a symlink cannot redirect the write)
#   7. Lua io.open flag writes must land via os.rename (symlink-safe;
...
for path in files:
    ...
    for i, line in enumerate(lines):
        if line.lstrip().startswith("//") or not re.search(r"FileView\s*\{", line):
            continue
        ...
        if not any(re.search(r"preload:\s*false", bl) for bl in block):
            fails.append(f"{path}:{i + 1}: FileView without preload: false — content must be read through BoundedFile, never FileView's own load")
