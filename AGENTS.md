...
- Root-owned files: `README.md`, `README.zh-Hant.md`, `LICENSE`, `manifest.json`, `BarWidget.qml`, `ToolPanel.qml`, `EvalQueue.qml`, `BoundedFile.qml`, `tabler-icons.ttf` (subset), `preview.png` (73:35 marketing cover), `panel.png` (raw panel screenshot), `design/cover.html` (cover source), `.github/workflows/*`, `scripts/check-submission.sh`, `scripts/check-hardening.sh`, `.gitignore`, and root-level project documentation.

## Local Contracts

- Plugin id `glasschan.oma-swiss`, kind `bar-widget`, single entry point `BarWidget.qml`.
- `BarWidget.qml` owns all state and actions; `ToolPanel.qml` is a pure view injected with `hostWidget` (consumes only the host members it names, via `t()` for strings). No state may live in the panel. The queued-eval slot lives in `EvalQueue.qml` — one module owning the single-slot queue, its eval runner and the completion notification; its interface is `enqueue(expr, notify, glyph)`. Bounded state-file reads live in `BoundedFile.qml` — one module wrapping the single-shot bounded nonblocking read (`timeout 5` deadline, `[ -f ]` regular-file gate, `dd iflag=nonblock bs=4096 count=1` byte cap); its interface is `fire()` → signal `read(content, exists)`, with last-wins requeue while a read is in flight. FileViews are pure change triggers (`preload: false`) — content never goes through FileView's own loader.
...
- Resource floor is a hard design rule: no timers (except the 100 ms configreload debounce), no polling, no background services. File watching via FileView `watchChanges` only (pure triggers, `preload: false`); content reads are change-triggered one-shots through `BoundedFile.qml`; hyprctl work through the single queued-eval slot in `EvalQueue.qml` so rapid clicks land in order. The swap/aspect flag writes/removals ride INSIDE the eval expressions (eval Lua carries io/os/math, verified 2026-08-24; flag writes land via unique-temp + `os.rename`) — the atomic flag-mutation invariant is encoded once in `flagEval()` (BarWidget.qml); file and compositor state move as one and the old setText/rm race (flag file disagreeing with live state until a reload reasserted the stale side) cannot recur; a rejected eval writes nothing.
...
- Security hardening round 2 (maintainer review of `331a883`, 2026-08-27 — the earlier "FileView reads are an accepted risk" stance was rejected by the marketplace): NO content read may go through FileView's own loader — every FileView sets `preload: false` and stays a pure change trigger; content reads ride `BoundedFile.qml` (regular-file gate + nonblocking open + 4 KB cap + 5 s deadline). Flag writes must be symlink-safe atomic replacements: shell writes go `mktemp` in the target directory + `mv -f` (rename replaces a planted link); eval-Lua writes go unique-temp-name + `os.rename` (`io.open "wx"` does not exist in hyprctl's Lua — verified 2026-08-27); `FileView.setText` with `atomicWrites` (default) is already temp+rename and stays the write path for lang/last-aspect/update-cache. The release-check response is size-bounded both ways: `curl --max-filesize 262144` + `| head -c 262144`. Known liveness quirk (accepted): if a symlink is planted at a watched path mid-session, QFileSystemWatcher arms on the link's target inode and that one watch goes stale until the next shell restart or config-reload broadcast — no hang, no memory growth, no redirected write; fixed watch re-arming is not worth adding polling.
- Security hardening follow-up (marketplace review of `209f221`, 2026-08-27): the update check must NEVER open a predictable lock file — an `exec N>` open truncates through a planted symlink. Mutual exclusion rides `flock` on the state directory's own inode (`exec 9<"${1%/*}"`, read-only fd, nothing created or truncated), and the legacy `update-check.lock` is unlinked in the same command (`rm -f` never follows a symlink). `check-hardening.sh` fails on any `exec N>` fd-redirection open.
...
- CI (`.github/workflows/ci.yml`): `validate` runs Omarchy's own `omarchy-plugin-validate`, fetched fresh from `basecamp/omarchy` branch `quattro` (`bin/omarchy-plugin-validate`) — the moving pin is intentional, so Omarchy manifest-schema drift fails CI instead of silently accepting a manifest the shell would reject. It also runs `scripts/check-submission.sh` (marketplace SUBMISSION.md rules: README install+removal+deps, LICENSE, preview ≤50 MB/40 MP) and `scripts/check-hardening.sh` (line-based tripwires for the security-baseline classes that arrived as marketplace comments on 2026-08-24 and 2026-08-27 — paths into `sh -c` must go through `shellQuote()`, hyprctl in a Process argv must sit behind coreutils `timeout`, `curl` must carry `--max-time` AND `--max-filesize`, every FileView must set `preload: false` (content reads belong to BoundedFile), no `cat >` redirection writes (mktemp + atomic mv instead), dd reads need timeout + `iflag=nonblock` + byte caps, Lua `io.open` writes need `os.rename` on the same or next line — plus the 書面語 rule for shipped Chinese; AGENTS.md itself is not scanned because the rule text quotes the markers). Release (`.github/workflows/release.yml`, on tag `v*`): tag must equal `manifest.json` `version`, then packages the plugin payload zip + sha256 and publishes the GitHub Release.
...
- Deploy loop: sync `BarWidget.qml`, `ToolPanel.qml`, `EvalQueue.qml`, `BoundedFile.qml`, `manifest.json`, `tabler-icons.ttf`, `preview.png`, `panel.png`, and docs to `~/.config/omarchy/plugins/glasschan.oma-swiss/`, ensure `~/.config/omarchy/shell.json` bar layout has `{"id": "glasschan.oma-swiss"}` (replacing `glasschan.super-alt-swap` if migrating), then `omarchy restart shell` (hot-reload can keep serving stale compiled QML), then E2E:
  ...
  - external writer check: from aspect off, run the stock `omarchy-hyprland-window-single-square-aspect-toggle` once; `status` must report `aspect=1:1` (watcher picked it up), then restore
  - bounded-read attacks (hardening round 2): plant a FIFO at the aspect flag path (`mkfifo`) — `status` must return instantly (no hang) with `aspect=off`, and the next `aspect w h` write atomically replaces the FIFO with a regular flag file; drop 100 KB of garbage at `~/.local/state/glasschan.oma-swiss/last-aspect` — `status` returns instantly and the shell stays healthy; plant a symlink at the pin path pointing at a sentinel file, then `pin` — the path must become a regular file (link replaced, not followed) and the sentinel untouched. Run the symlink test LAST: it leaves that one watch stale until the next shell restart (known quirk), so restart the shell afterwards.
- End of test session: restore the user's pre-test state exactly (capture `omarchy-shell glasschan.oma-swiss status` before testing and diff against it at the end); leave Opinionated Looks as found.
