...
- End of test session: capture `omarchy-shell glasschan.oma-swiss status` before testing and diff against it at the end — currently swap off, aspect 4:3, pin on, look on, lang zh; leave Opinionated Looks as the user's current visual state (on, via the flag file).
